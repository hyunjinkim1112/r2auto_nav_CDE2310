...
//Added std messages for status and command
#include "std_msgs/msg/string.hpp"
#include <chrono> //Added for checking time elapsed for goal completion
...
public:
    AutoMapper()
        ...
        // Susbcriptions required for detecting frontiers 
        ...
        // Publisher to visualize frontiers in Rviz
        markerArrayPublisher_ = create_publisher<MarkerArray>("/frontiers", 10);
        poseNavigator_ = rclcpp_action::create_client<NavigateToPose>(
                this,
                "/navigate_to_pose"); 

        poseNavigator_->wait_for_action_server(); // Wait for the action server to be available before starting
        ...
        //Create subscriber and publisher for status and command messages to communicate with coordinator_node
...
private:
    const double MIN_FRONTIER_DENSITY = 0.1; // Number of neighbouring frontier cells around a cell to be considered a frontier
    const double MIN_DISTANCE_TO_FRONTIER_SQUARED = pow(0.65,2); 
    const int MIN_FREE_THRESHOLD = 2; //Minimum number of free cells  around a ceel to be considered a frontier
    const int MAX_OBSTACLE_THRESHOLD = 1; //Maximum number of obstacle cells around a cell to not be considered a frontier
    Costmap2D costmap_;
    rclcpp_action::Client<NavigateToPose>::SharedPtr poseNavigator_; // Shared pointer to the action client
    ...
    int markerId_;
    string mapPath_;
    // Store number of exploration attempts failed for r2autonav backup
    int exploration_attempts_failed = 0;
    // Subscriber and publisher for status and command messages
    ...
    // Added explore start check to ensure explore() does not run when not needed
    ...
    rclcpp::TimerBase::SharedPtr timer_;  // Timer to handle extremely quick goal completion
    ...
    static array<unsigned char, 256> initTranslationTable() {
        ...
        // Nav2 Costmap lineary mapped from [0..100] to [0..255]
        ...
        /* Attempt to classify higher cost cells as lethal (Failed but kept for future reference)
    ...
    struct Frontier { // Defining the structure of a Frontier object
        Point centroid;
        vector<Point> points;
    ...
    void statusTopicCallback(std_msgs::msg::String::SharedPtr message) {
        // status_ = move(message);  // For testing
        RCLCPP_INFO(get_logger(), "statusTopicCallback...");
        if (message->data == "START") { //Once start command from coordinator_node is received, start exploration
    ...
    void updateFullMap(OccupancyGrid::UniquePtr occupancyGrid) {
        ...
        // Lock costmap thread as we are accessing raw underlying map
        auto *mutex = costmap_.getMutex();
        lock_guard<Costmap2D::mutex_t> lock(*mutex);

        // fill map with data
        unsigned char *costmap_data = costmap_.getCharMap();
        size_t costmap_size = costmap_.getSizeInCellsX() * costmap_.getSizeInCellsY(); // get size of costmap for translation
    ...
    // Function for sending Marker messages to Rviz for frontier visualization
    ...
    // Function to clear markers from Rviz before sending new ones
    // This is done to avoid cluttering the Rviz with old frontier markers
    void clearMarkers() {
    ...
    void stop() {
        ...
        // reset subscriptions to topics to minimize any errors like not receiving the latest data
        poseSubscription_.reset(); 
        mapSubscription_.reset(); 
        poseNavigator_->async_cancel_all_goals();
        //saveMap(); //uncomment if you want to save map for future use
        clearMarkers();  
    }
    void restart() {
        RCLCPP_INFO(get_logger(), "Restarting...");
        // Reinitialize the susbcriptions to start getting data again
        poseSubscription_ = create_subscription<PoseWithCovarianceStamped>(
                "/pose", 10, bind(&AutoMapper::poseTopicCallback, this, _1));
    ...
    // The below random movement was purely used for testing purposes
    ...
    void explore() {
        //Check whether exploration has been stopped
        if (!explore_start) { 
            RCLCPP_INFO(get_logger(), "Explore stopped");
            return; 
        }
        if (isExploring_) { return; } // If already exploring, do not start frontier detection again in order to avoid cluttering the map
        auto frontiers = findFrontiers();
        // If exploration attempts failed more than 5 times, send a message to coordinator node to initiate r2autonav.
        if (exploration_attempts_failed > 5) {
            ...
            geometry_msgs::msg::Twist move_cmd;
            // Firstly stop the robot to prevent cmd_vel topic thread-lock from nav2 action server. However, unsure whether this prevents thread-lock
            ...
            stop(); // Stop and restart susbcriptions to minimize and resolve errors in data reception
            restart();
            exploration_attempts_failed = 0; // Reset the exploration attempts failed counter
            return;
        }
        if (frontiers.empty()) {
            exploration_attempts_failed++; // Increment the exploration attempts failed counter when no frontiers are found
        ...
        // If failed goal increment greater than 3, stop and restart (This is no longer used as it does not seem very effective, but it is kept for future reference)
        // If you wish to try using these, you will need to initialize failed_goal_index_increment as a private variable
        // What this does is that it will shift the goal to a different nearby frontier if the previous goal failed
        /*
        ...
        const auto frontier = frontiers[0]; // Choose the first frontier to explore
        drawMarkers(frontiers); //Visualize the frontiers on Rviz
        auto goal = NavigateToPose::Goal(); //Set goal to the first frontier
        ...
        auto send_goal_options = rclcpp_action::Client<NavigateToPose>::SendGoalOptions(); // Send goal to Action Client
        send_goal_options.goal_response_callback = [this, &frontier](
                const GoalHandleNavigateToPose::SharedPtr &goal_handle) {
            if (goal_handle) {
                RCLCPP_INFO(get_logger(), "Goal accepted by server, waiting for result"); // If successful, start exploration and counting time elapsed
                isExploring_ = true;
                goal_start_time_ = std::chrono::steady_clock::now();
            } else {
                RCLCPP_ERROR(get_logger(), "Goal was rejected by server"); // Otherwise increment the exploration attempts failed counter in order to pass handle to r2autonav
        ...
        send_goal_options.feedback_callback = [this](
            ...
            RCLCPP_INFO(get_logger(), "Distance remaining: %f", feedback->distance_remaining); //Check distance remaining for goal completion
        };
        ...
        send_goal_options.result_callback = [this](const GoalHandleNavigateToPose::WrappedResult &result) {
            ...
            // timer used to check elapsed time in the case that the goal is reached too quickly which usually means that there is an error with the Navigation 2 stack
            if (elapsed_time < 1) {
            ...
            switch (result.code) {
                case rclcpp_action::ResultCode::SUCCEEDED:
                    ...
                    stop();
                    restart();
                    //poseNavigator_->async_cancel_all_goals(); // haven't tested this fully yet, but doesn't seem to work in resolving nav2 issues 
                    break;
    ...
    // Function to get the 8-connected neighborhood of a given cell index
    vector<unsigned int> nhood8(unsigned int idx) {
        ...
        const int x = mx;
        const int y = my;
        // using pairs to represent the 8-connected neighborhood
        ...
        for (const auto &d: directions) {
            int newX = x + d.first;
            int newY = y + d.second;
            // check that neighbourhood cell is within bounds of costmap before adding it to out
    ...
    bool isAchievableFrontierCell(unsigned int idx,
        ...
        // check that cell is not adjacent to a lethal obstacle
        int obstacleCount = 0;
        ...
        //check whether there's enough free space for robot to move to the cell
        int freeCount = 0;
        for (unsigned int nbr: nhood8(idx)) {
        ...
        while (!bfs.empty()) {
            ...
            for (unsigned int nbr: nhood8(idx)) {
                ...
                if (isAchievableFrontierCell(nbr, frontier_flag)) {
                    ...
                    bfs.push(nbr); //add to queue for frontier using breadth-first search
                }
            }
        }
    ...
    vector<Frontier> findFrontiers() {
        ...
        // make sure costmap is consistent and topic is locked for duration of search
        ...
        while (!bfs.empty()) {
            ...
            for (unsigned nbr: nhood8(idx)) {
                ...
                if (map_[nbr] == FREE_SPACE && !visited_flag[nbr]) {
                    ...
                    //Check whether the frontier has atleast another frontier as a neighbour so single frontier cells are not considered
